...
function read_sensor_data($consequent_id){
	...
	else
	{
		...
		while (!feof($file_handle))
		{
			...
			if($line_counter==0){
				$data=explode(',', $line);
				$element_header = array_fill_keys($data, 0);
			}else{
				$data=explode(',', $line);
				$element_list[] = $data;
			}
		...
		fclose($file_handle);
		$element_list=flip_array($element_list);
// 		print_r($element_header);
// 		print_r($element_list);
		$i=0;
		foreach($element_header as $x => $x_value) {
// 			echo "Key=" . $x . ", Value=" . $x_value;
			$element_header[$x]=$element_list[$i];
// 			echo "<br>";
			$i++;
		}
// 		$element_list=array_combine($element_header,$element_list);
// 		print_r($element_header);
		return $element_header;
	}
}
function read_sensor_data_old($consequent_id){
		if(!file_exists("textfiles/".$consequent_id.".txt"))
			{
				// 		die("File not found");
				return 'FNF';
...
for($i=0;$i<$rows;$i++){
	...
	write_to_log(__FILE__,__LINE__,__FUNCTION__,'after flip',$out);
	return $out;
}
function merge_array($data,$child_data){
...
function merge_array_ass($data,$child_data){
// 	echo'$data_ass<br>';
// 	print_r($data);
// 	echo'$$child_data_ass<br>';
// 	print_r($child_data);
	foreach($child_data as $x => $x_value) {
...
// 				$data = merge_array($data,$child_data);
		...
		$rows = count($data);
		$cols = count($data[0]); // assumes non empty matrix
...
function m_data_normalization($dataNS){
	...
	for($i=0;$i<count($dataNS);$i++){
		for($j=0;$j<count($dataNS[0]);$j++)
		$data[$i][j]=($dataNS[$i]-min($dataNS))/(max($dataNS)-min($dataNS));
...
// 	write_to_log(__FILE__,__LINE__,__FUNCTION__,'min($dataNS)',min($dataNS));
	for($i=0;$i<count($dataNS);$i++){
		$dataNS[$i]=($dataNS[$i]-min($dataNS))/(max($dataNS)-min($dataNS));
// 		write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS['.$i.']',$dataNS[$i]);
// 		$dataNS[$i]=$dataNS[$i]*100;
// 		write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS['.$i.']',$dataNS[$i]);
...
// 			if( $_GET['countofcall'] ==0){
				...
				//write_to_log(__FILE__,__LINE__,__FUNCTION__,' $count', $count);
					...
					$result_status=perform_calculate_sensor_data();
					if($result_status=='done'){
						
						$data['numberOfGraph']=$_POST['number_of_child'];
						$s=$_SESSION[$_GET['node']]['horizontalAxisLabel'];
// 						$data['sensor_data_result']=array();
// 						$data['sensor_graph_label']=array();
						$sensor_data_result=array();
						$sensor_graph_label=array();
 						$data['horizontalAxisLabel']=implode(",",$s);
						for($ci=1;$ci<=$_POST['number_of_child'];$ci++){
// 							$data['horizontalAxisLabel_'.$ci]=$_SESSION[$_GET['node']]["horizontalAxisLabel"][$ci-1];
// 							$data['sensor_data_result_'.$ci]=implode(",",$_SESSION[$_GET['node']]["sensor_data_result_".$ci]);
// 							$data['sensor_graph_label_'.$ci]=$_SESSION[$_GET['node']]["sensor_graph_label_".$ci];
							array_pop($_SESSION[$_GET['node']]['sensor_data_result_'.$ci]);
//  							$data['sensor_data_result'][]='['.implode(",",$_SESSION[$_GET['node']]['sensor_data_result_'.$ci]).']';
//  							$data['sensor_graph_label'][]='['.implode(",",$_SESSION[$_GET['node']]['sensor_graph_label_'.$ci]).']';
 							$sensor_data_result[]=implode(",",$_SESSION[$_GET['node']]['sensor_data_result_'.$ci]);
 							$sensor_graph_label[]=implode(",",$_SESSION[$_GET['node']]['sensor_graph_label_'.$ci]);
						}
						$data['sensor_data_result']=implode('__DIVIDER__', $sensor_data_result);
						$data['sensor_graph_label']=implode('__DIVIDER__', $sensor_graph_label);
						$data['sensor_graph_label_output']=$_GET['node'];
// 						$data['sensor_data_result']=$sensor_data_result;
// 						$data['sensor_graph_label']=$sensor_graph_label;
// 						write_to_log(__FILE__,__LINE__,__FUNCTION__,'$data[sensor_data_result]',$data['sensor_data_result']);
// 						write_to_log(__FILE__,__LINE__,__FUNCTION__,'$data[sensor_graph_label]',$data['sensor_graph_label']);
// 						$data['sensor_data_result']='['.implode(",",$sensor_data_result).']';
// 						$data['sensor_graph_label']='['.implode(",",$sensor_graph_label).']';
// 						write_to_log(__FILE__,__LINE__,__FUNCTION__,'$data[sensor_data_result]',$data['sensor_data_result']);
// 						write_to_log(__FILE__,__LINE__,__FUNCTION__,'$data[sensor_graph_label]',$data['sensor_graph_label']);
						$data['returntag_status'] = '.status';
					...
					if($result_data[count($result_data)-1]=='end'){
						...
						//print_r($result_data);
						// 				sort($result_data);
						$graph_label=$_SESSION[$_GET['node']]["sensor_graph_label"];
						// 				sort($graph_label);
						$tmp='.status';
						
						
						$data['graph_number']=$_SESSION[$_GET['node']]['$noOfantecedent'];
						for($c=0;$c<$_SESSION[$_GET['node']]['$noOfantecedent'];$c++){
							$data['graph_data_'.$c] = implode(',',$result_data);
							$data['graph_label_'.$c] = implode(',',$graph_label);
							$data['horizontalAxisLabel_'.$c] = $_POST['antecedent_id_'.];
						} 
						$data['returntag'] = $tmp;
						$data['returntag_status'] = '.status';
						$data['horizontalAxisLabel'] = $_SESSION[$_GET['node']]['horizontalAxisLabel'];
						$html_status='Performed Computation for sensor data';
						$data['end_of_sensor_data'] = 'done';
